...
class Message {
    ...
    public function getConversation($user1, $user2) {
        global $pdo;
        // On récupère les messages dans l'ordre chronologique (ASC)
        $stmt = $pdo->prepare("
    ...
    public function getContacts($user_id) {
        global $pdo;
        // On récupère tous les utilisateurs avec qui l'utilisateur a eu une conversation
        $stmt = $pdo->prepare("
            SELECT DISTINCT u.id, u.nom, u.prenom, u.avatar, 
