...
class Post {
    ...
    public function getTousLesPosts() {
        global $pdo;
        // Jointure avec la table "users" - ajout de photo_profil
        $stmt = $pdo->query("
            SELECT posts.id, posts.contenu, posts.created_at, posts.is_pinned,
                   users.id as user_id, users.nom, users.prenom, users.filiere, users.promotion, users.photo_profil
            FROM posts 
    ...
    public function getCommentaires($post_id) {
        ...
        $stmt = $pdo->prepare("
            SELECT comments.id, comments.contenu, comments.created_at,
                   users.id as user_id, users.nom, users.prenom, users.filiere, users.photo_profil
            FROM comments
